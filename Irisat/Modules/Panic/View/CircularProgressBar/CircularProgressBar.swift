...
protocol PanicDidEndDelegate: class {
...
class CircularProgressBar: UIView {
  ...
  weak var delegate: PanicDidEndDelegate?
  ...
  public var labelIsHiden: Bool = false {
    didSet{
      label.isHidden = labelIsHiden
    }
  }
  ...
  public func setProgress(to progressConstant: Double, withAnimation: Bool) {
    ...
    let timer = Timer.scheduledTimer(withTimeInterval: 0.05, repeats: true) { (timer) in
      if currentTime >= 2{
        timer.invalidate()
        self.delegate?.animation(didEnd: true)
      } else {
        currentTime += 0.05
  ...
  private func setForegroundLayerColorForSafePercent(){
    if Int(label.text!)! >= self.safePercent {
      ...
      delegate?.animation(didBegin: true)
    }
  }
